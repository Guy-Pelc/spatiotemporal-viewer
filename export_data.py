#!/usr/bin/env python3
"""Pack the .h5ad into compact files the browser viewer can load.

Outputs into viewer/data/:
...
SRC = os.path.join(HERE, "coronal_downsampled.h5ad")
...
# gene-major so one gene's column is contiguous
Xq_gm = np.ascontiguousarray(Xq.T)  # shape (genes, n)
Xq_gm.tofile(os.path.join(OUT, "expr.bin"))

# raw per-gene max (pre-log) for tooltip display
...
categoricals = {c: cat(c) for c in
                ["celltype", "region", "subregion", "batch", "mouse_id", "slide_id"]}

def num(col):
    return np.asarray(a.obs[col], dtype=np.float32).tolist()

numeric = {c: num(c) for c in
           ["age", "transcript_count", "num_detected_genes", "volume"]}
